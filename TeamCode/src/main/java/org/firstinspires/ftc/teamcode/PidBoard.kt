...
class PidBoard {
    ...
    private val pid: Array<PID?> = arrayOfNulls(4)
    @JvmOverloads
    fun getHW(hwMap: HardwareMap, telemetry: Telemetry? = null) {
        ...
        try {
            ...
            intakeServo?.direction = DcMotorSimple.Direction.FORWARD
        } catch (_: Throwable) {
        ...
        try {
            ...
            hook!!.direction = DcMotorSimple.Direction.FORWARD
        } catch (_: Throwable) {
            broken.add("Hook")
        }
        ...
        try{
            for(i in driveBase.indices){
                pid[i] = PID(5.0, 0.0, 0.0, {driveBase[i]!!.velocity})
            }
        } catch(_ : Throwable){
            broken.add("The PIDs")
        }
